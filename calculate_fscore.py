...
def main():
    ...
    for data_i in data_list:
        ...
        condition_list.append(f'{mode}_{data_name}_{M}_{DS_dim}_{PS_dim}_{cano_type}_{lag_i}')
        #print(in_dir)
        #各条件での評価
        for image_name in os.listdir(in_dir):
            ...
            mode1 = "direc_standard"
            in_dir1 = f'{parentpath1(__file__, f=0)}/dissim_{mode1}_{data_name}_{M}_{DS_dim}_{PS_dim}_{cano_type}_{lag_i}_train'
            pred_dir = pd.read_csv(f'{in_dir1}/{data_name}_{mode1}_w{w}_o{o}_l{lag_i}_d{M}_dsdim{DS_dim}_psdim{PS_dim}_top{cano_type}_s2_train.csv', names=["A"])
            dir_list.append(f'{in_dir1}/{data_name}_{mode1}_w{w}_o{o}_l{lag_i}_d{M}_dsdim{DS_dim}_psdim{PS_dim}_top{cano_type}_s2_train.csv')
            ...
            max8.append(auc8)
            count += 1
        new_dir_path1 = f'fscore_{mode}_eachdata/fscore_{mode}_{data_name}_train'
        os.makedirs(new_dir_path1, exist_ok=True)
        df = pd.DataFrame(auc_all, index = trajectory_list, columns = ['0', 'W', 'O', 'L', 'W+O', 'W+L', 'O+L', 'W+O+L'])
        #df.to_csv(f'{new_dir_path1}/fscore_{mode}_{data_name}_{M}_{DS_dim}_{PS_dim}_{cano_type}_{lag_i}_train.csv')
        #print(count)
        ...
        pred = pd.read_csv(in_dir+"/"+trajectory_list[np.argmax(max7)], names=["A"])
        pred = pred['A'].fillna(pred['A'].mean())
        pred = pred.values

        pred_dir = pd.read_csv(dir_list[np.argmax(max7)]).values
    ...
    #df_ave.to_csv('fscore_'+mode+'_train_ave.csv')
    #df_all.to_csv('fscore_'+mode+'_train_max.csv')
    #print("")
